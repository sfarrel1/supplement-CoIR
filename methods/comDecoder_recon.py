...
from modules import comDecoder

class ComDecoderRecon:
    ...
    def norm_lpip(self, x):
        # Normalizes image between [-1,1]
        return 2 * ((x - x.min()) / (x.max() - x.min())) - 1

    def norm_mean_std(self, img1, img2):
        # mean stdev normalization so ground truth matches recon.
        # img1: ground truth
        # img2: reconstruction
    ...
    def recon(self,gt_datacube,full_datacube,sparse_datacube,
        ...
        out_size = [H, W]  # network output size
        upsample_mode = "nearest"  # upsampling mode (bilinear, nearest)

        # create random initialized network
        inp = get_noise(input_channels, 'noise', in_size, noise_type='u', var=1/10).to(self.device)
        model = comDecoder.com_decoder(num_layers, input_channels, num_channels, output_channels,
                                        out_size, in_size, kernel_size, upsample_mode).to(self.device)
        ...
        # scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lambda xx: 0.1 ** min(xx / niters, 1))
        ...
        gt = torch.abs(gt_heatmap).cpu().squeeze().numpy()
        best_im_crop = best_im[:, :, :, rng_st:N_rng].detach()
        recon = torch.abs(best_im_crop).cpu().squeeze().numpy()
        gt = self.norm(gt)
        ...
        torch.save(inp.detach().cpu(), os.path.join(save_dir, 'noise_inp.pt'))
