...
from modules.utils import TV, L1_reg, L2_reg, psnr, prop_plane, delay_sum
...
class INRRecon:
    ...
    def recon(self,gt_datacube,full_datacube,sparse_datacube,
              ...
              mask,save_dir,DATA_PARAMS,lr,
              niters,reg,reg_weight,model_name,
              ffenc=False,alpha=30,omega0=30,nonlin='relu',
              map_feat=128,view_recon=False):
        ...
        # -----------------------------------------------------------------------------------------------------
        H = int(N_ang); W = int(N_rng)

        # MLP based implicit NN architectures
        model = mlp.INR(in_features=2, hidden_features=256, hidden_layers=6,
                        out_features=2, nonlinearity=nonlin, outermost_linear=True,
                        first_omega_0=omega0, hidden_omega_0=30, scale=alpha,
                        ff_encode=ffenc, mapping_size=map_feat).to(self.device)
        ...
        inp = torch.hstack((X.reshape(-1, 1), Y.reshape(-1, 1)))[None, ...]
        ...
        for idx in tqdm.tqdm(range(niters)):
            ...
            pred = torch.view_as_complex(model(inp)).reshape(-1, H, W)[None, ...]
