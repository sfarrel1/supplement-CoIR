...
from modules.utils import TV, L1_reg, L2_reg, grad_reg, psnr, prop_plane, delay_sum, get_inp
...
class GradDescRecon:
    ...
    def recon(self, gt_datacube,full_datacube,sparse_datacube,
        ...
        assert reg in ['none', 'l1', 'l2', 'tv', 'grad_reg']
        ...
        # -----------------------------------------------------------------------------------------------------
        H = N_ang; W = N_rng
        pred = get_inp([1, 1, H, W], const=1/10, dtype=torch.cfloat, device=self.device)
        model_parameters = [pred]
        optim_dict = [
            {'params': model_parameters, 'lr': lr, 'eps': 1e-08}
        ]
