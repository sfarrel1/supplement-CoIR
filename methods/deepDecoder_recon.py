...
from modules import deepDecoder

class DeepDecoderRecon:
    ...
    def recon(self,gt_datacube,full_datacube,sparse_datacube,
        ...
        # Reinhard Heckel deep decoder NN for compressed sensing inverse problems
        ...
        input_channels = 512  # number of input channels
        num_channels = 512  # number of channels per hidden layer
        output_channels = 2  # number of output channels
        out_size = [H, W]  # network output size
        upsample_mode = "bilinear"  # upsample mode in NN
        ...
        model = deepDecoder.deep_decoder(num_layers, input_channels, num_channels, output_channels,
                                         out_size, in_size, upsample_mode=upsample_mode).to(self.device)
        ...
        mdic1 = {"psnr": np.array(psnr_m), "ssim": np.array(ssim_m), "loss": np.array(H_out["train_loss"]),
                 "mae": np.array(mae_m), "runTime": np.array(totTime)}
